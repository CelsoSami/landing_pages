...
# MAGIC %md
# MAGIC # Transformação EBA_CSC - Aba Pgto
...
# MAGIC %md
# MAGIC ## Leitura da Aba Pgto

# COMMAND ----------

fonte_pgto = pd.read_excel(
    CAMINHO_ARQUIVO,
    sheet_name="Pgto",
    header=None
)
...
pos_tabela_precos = find_cell(fonte_pgto, "TABELA DE PREÇOS NO PERIODO")

if pos_tabela_precos is None:
    raise ValueError("Texto 'TABELA DE PREÇOS NO PERIODO' não encontrado na aba Pgto")
...
servicos = []
for i in range(linha_inicio_servicos, len(fonte_pgto)):
    srv_val = fonte_pgto.iat[i, coluna_servico]
    if pd.isna(srv_val) or str(srv_val).strip() == "":
        break
    servicos.append({
        "Servico": str(srv_val).strip(),
        "Peso": safe_number(fonte_pgto.iat[i, coluna_peso])
    })
...
pos_rotulos = find_cell(fonte_pgto, "Rótulos de Linha")

if pos_rotulos is None:
    raise ValueError("Texto 'Rótulos de Linha' não encontrado na aba Pgto")

linha_pivot = pos_rotulos[0]
coluna_pivot = pos_rotulos[1]

linha_cabecalho = fonte_pgto.iloc[linha_pivot].values
servicos_pivot = linha_cabecalho[coluna_pivot + 1:]

centros_custo = []
for i in range(linha_pivot + 1, len(fonte_pgto)):
    cc_val = fonte_pgto.iat[i, coluna_pivot]
    if pd.isna(cc_val):
        continue
    ...
    centros_custo.append({
        "CC": str(cc_val).strip(),
        "row_data": fonte_pgto.iloc[i].values
    })
...
pos_formacao = find_cell(fonte_pgto, "FORMAÇÃO DE PREÇOS NO PERIODO")
...
formacao_precos = []
for i in range(linha_formacao_servicos, len(fonte_pgto)):
    srv_val = fonte_pgto.iat[i, coluna_formacao_servico]
    if pd.isna(srv_val) or str(srv_val).strip() == "":
        break
    formacao_precos.append({
        "Servico": str(srv_val).strip().upper(),
        "Peso_Formacao": safe_number(fonte_pgto.iat[i, coluna_formacao_peso])
    })
...
pos_volumetria = find_cell(fonte_pgto, "VOLUMETRIA NO PERIODO")
...
servicos_vol_raw = fonte_pgto.iloc[linha_servicos_vol, coluna_inicio_vol:].values
volumetria_raw = fonte_pgto.iloc[linha_volumetria, coluna_inicio_vol:].values
volumetria_peso_raw = fonte_pgto.iloc[linha_volumetria_peso, coluna_inicio_vol:].values
...
# MAGIC %md
# MAGIC ## CUSTO DE PGTO (Validação)

# COMMAND ----------

valor_custo_pgto = get_value_after_label(fonte_pgto, "CUSTO DE PGTO:")

soma_valor_calculado = tabela_final["Valor"].sum()

if valor_custo_pgto is not None:
    valor_custo_num = safe_number(valor_custo_pgto)
    validacao = (
...
print(f"Custo de PGTO: {valor_custo_pgto}")
...
tabela_final["ValidacaoPgto"] = validacao
...
checklist = {
    ...
    "CUSTO DE PGTO encontrado": valor_custo_pgto is not None,
...
# tabela_final.write.format("delta").mode("overwrite").saveAsTable("tabela_pgto")
