...
# MAGIC %md
# MAGIC # Transformação EBA_CSC - Aba GED
...
# MAGIC %md
# MAGIC ## Leitura da Aba GED

# COMMAND ----------

fonte_ged = pd.read_excel(
    CAMINHO_ARQUIVO,
    sheet_name="GED",
    header=None
)
...
pos_tabela_precos = find_cell(fonte_ged, "TABELA DE PREÇOS NO PERIODO")

if pos_tabela_precos is None:
    raise ValueError("Texto 'TABELA DE PREÇOS NO PERIODO' não encontrado na aba GED")
...
servicos = []
for i in range(linha_inicio_servicos, len(fonte_ged)):
    srv_val = fonte_ged.iat[i, coluna_servico]
    if pd.isna(srv_val) or str(srv_val).strip() == "":
        break
    servicos.append({
        "Servico": str(srv_val).strip(),
        "Peso": safe_number(fonte_ged.iat[i, coluna_peso])
    })
...
pos_rotulos = find_cell(fonte_ged, "Rótulos de Linha")

if pos_rotulos is None:
    raise ValueError("Texto 'Rótulos de Linha' não encontrado na aba GED")

linha_pivot = pos_rotulos[0]
coluna_pivot = pos_rotulos[1]

linha_cabecalho = fonte_ged.iloc[linha_pivot].values
servicos_pivot = linha_cabecalho[coluna_pivot + 1:]

centros_custo = []
for i in range(linha_pivot + 1, len(fonte_ged)):
    cc_val = fonte_ged.iat[i, coluna_pivot]
    if pd.isna(cc_val):
        continue
    ...
    centros_custo.append({
        "CC": str(cc_val).strip(),
        "row_data": fonte_ged.iloc[i].values
    })
...
pos_formacao = find_cell(fonte_ged, "FORMAÇÃO DE PREÇOS NO PERIODO")
...
formacao_precos = []
for i in range(linha_formacao_servicos, len(fonte_ged)):
    srv_val = fonte_ged.iat[i, coluna_formacao_servico]
    if pd.isna(srv_val) or str(srv_val).strip() == "":
        break
    formacao_precos.append({
        "Servico": str(srv_val).strip().upper(),
        "Peso_Formacao": safe_number(fonte_ged.iat[i, coluna_formacao_peso])
    })
...
pos_volumetria = find_cell(fonte_ged, "VOLUMETRIA NO PERIODO")
...
servicos_vol_raw = fonte_ged.iloc[linha_servicos_vol, coluna_inicio_vol:].values
volumetria_raw = fonte_ged.iloc[linha_volumetria, coluna_inicio_vol:].values
volumetria_peso_raw = fonte_ged.iloc[linha_volumetria_peso, coluna_inicio_vol:].values
...
# MAGIC %md
# MAGIC ## CUSTO DE GED (Validação)

# COMMAND ----------

valor_custo_ged = get_value_after_label(fonte_ged, "CUSTO DE GED:")

soma_valor_calculado = tabela_final["Valor"].sum()

if valor_custo_ged is not None:
    valor_custo_num = safe_number(valor_custo_ged)
    validacao = (
...
print(f"Custo de GED: {valor_custo_ged}")
...
tabela_final["ValidacaoGED"] = validacao
...
checklist = {
    ...
    "CUSTO DE GED encontrado": valor_custo_ged is not None,
...
# tabela_final.write.format("delta").mode("overwrite").saveAsTable("tabela_ged")
