...
# MAGIC %md
# MAGIC # Transformação EBA_CSC - Aba Protocolo
...
# MAGIC %md
# MAGIC ## Leitura da Aba Protocolo

# COMMAND ----------

fonte_protocolo = pd.read_excel(
    CAMINHO_ARQUIVO,
    sheet_name="Protocolo",
    header=None
)
...
pos_tabela_precos = find_cell(fonte_protocolo, "TABELA DE PREÇOS NO PERIODO")

if pos_tabela_precos is None:
    raise ValueError("Texto 'TABELA DE PREÇOS NO PERIODO' não encontrado na aba Protocolo")
...
servicos = []
for i in range(linha_inicio_servicos, len(fonte_protocolo)):
    srv_val = fonte_protocolo.iat[i, coluna_servico]
    if pd.isna(srv_val) or str(srv_val).strip() == "":
        break
    servicos.append({
        "Servico": str(srv_val).strip(),
        "Peso": safe_number(fonte_protocolo.iat[i, coluna_peso])
    })
...
pos_rotulos = find_cell(fonte_protocolo, "Rótulos de Linha")

if pos_rotulos is None:
    raise ValueError("Texto 'Rótulos de Linha' não encontrado na aba Protocolo")

linha_pivot = pos_rotulos[0]
coluna_pivot = pos_rotulos[1]

linha_cabecalho = fonte_protocolo.iloc[linha_pivot].values
servicos_pivot = linha_cabecalho[coluna_pivot + 1:]

centros_custo = []
for i in range(linha_pivot + 1, len(fonte_protocolo)):
    cc_val = fonte_protocolo.iat[i, coluna_pivot]
    if pd.isna(cc_val):
        continue
    ...
    centros_custo.append({
        "CC": str(cc_val).strip(),
        "row_data": fonte_protocolo.iloc[i].values
    })
...
pos_formacao = find_cell(fonte_protocolo, "FORMAÇÃO DE PREÇOS NO PERIODO")
...
formacao_precos = []
for i in range(linha_formacao_servicos, len(fonte_protocolo)):
    srv_val = fonte_protocolo.iat[i, coluna_formacao_servico]
    if pd.isna(srv_val) or str(srv_val).strip() == "":
        break
    formacao_precos.append({
        "Servico": str(srv_val).strip().upper(),
        "Peso_Formacao": safe_number(fonte_protocolo.iat[i, coluna_formacao_peso])
    })
...
pos_volumetria = find_cell(fonte_protocolo, "VOLUMETRIA NO PERIODO")
...
servicos_vol_raw = fonte_protocolo.iloc[linha_servicos_vol, coluna_inicio_vol:].values
volumetria_raw = fonte_protocolo.iloc[linha_volumetria, coluna_inicio_vol:].values
volumetria_peso_raw = fonte_protocolo.iloc[linha_volumetria_peso, coluna_inicio_vol:].values
...
# MAGIC %md
# MAGIC ## CUSTO DE PROTOCOLO (Validação)

# COMMAND ----------

valor_custo_protocolo = get_value_after_label(fonte_protocolo, "CUSTO DE PROTOCOLO:")

soma_valor_calculado = tabela_final["Valor"].sum()

if valor_custo_protocolo is not None:
    valor_custo_num = safe_number(valor_custo_protocolo)
    validacao = (
...
print(f"Custo de Protocolo: {valor_custo_protocolo}")
...
tabela_final["ValidacaoProtocolo"] = validacao
...
checklist = {
    ...
    "CUSTO DE PROTOCOLO encontrado": valor_custo_protocolo is not None,
...
# tabela_final.write.format("delta").mode("overwrite").saveAsTable("tabela_protocolo")
