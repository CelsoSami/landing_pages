...
# MAGIC %md
# MAGIC # Transformação EBA_CSC - Aba Cep
...
# MAGIC %md
# MAGIC ## Leitura da Aba Cep

# COMMAND ----------

fonte_cep = pd.read_excel(
    CAMINHO_ARQUIVO,
    sheet_name="Cep",
    header=None
)
...
pos_tabela_precos = find_cell(fonte_cep, "TABELA DE PREÇOS NO PERIODO")

if pos_tabela_precos is None:
    raise ValueError("Texto 'TABELA DE PREÇOS NO PERIODO' não encontrado na aba Cep")
...
servicos = []
for i in range(linha_inicio_servicos, len(fonte_cep)):
    srv_val = fonte_cep.iat[i, coluna_servico]
    if pd.isna(srv_val) or str(srv_val).strip() == "":
        break
    servicos.append({
        "Servico": str(srv_val).strip(),
        "Peso": safe_number(fonte_cep.iat[i, coluna_peso])
    })
...
pos_rotulos = find_cell(fonte_cep, "Rótulos de Linha")

if pos_rotulos is None:
    raise ValueError("Texto 'Rótulos de Linha' não encontrado na aba Cep")

linha_pivot = pos_rotulos[0]
coluna_pivot = pos_rotulos[1]

linha_cabecalho = fonte_cep.iloc[linha_pivot].values
servicos_pivot = linha_cabecalho[coluna_pivot + 1:]

centros_custo = []
for i in range(linha_pivot + 1, len(fonte_cep)):
    cc_val = fonte_cep.iat[i, coluna_pivot]
    if pd.isna(cc_val):
        continue
    ...
    centros_custo.append({
        "CC": str(cc_val).strip(),
        "row_data": fonte_cep.iloc[i].values
    })
...
pos_formacao = find_cell(fonte_cep, "FORMAÇÃO DE PREÇOS NO PERIODO")
...
formacao_precos = []
for i in range(linha_formacao_servicos, len(fonte_cep)):
    srv_val = fonte_cep.iat[i, coluna_formacao_servico]
    if pd.isna(srv_val) or str(srv_val).strip() == "":
        break
    formacao_precos.append({
        "Servico": str(srv_val).strip().upper(),
        "Peso_Formacao": safe_number(fonte_cep.iat[i, coluna_formacao_peso])
    })
...
pos_volumetria = find_cell(fonte_cep, "VOLUMETRIA NO PERIODO")
...
servicos_vol_raw = fonte_cep.iloc[linha_servicos_vol, coluna_inicio_vol:].values
volumetria_raw = fonte_cep.iloc[linha_volumetria, coluna_inicio_vol:].values
volumetria_peso_raw = fonte_cep.iloc[linha_volumetria_peso, coluna_inicio_vol:].values
...
valor_custo_pgt = get_value_after_label(fonte_cep, "CUSTO DE PGTO:")

soma_valor_calculado = tabela_final["Valor"].sum()

if valor_custo_pgt is not None:
    valor_custo_num = safe_number(valor_custo_pgt)
    validacao = (
...
print(f"Custo de PGTO: {valor_custo_pgt}")
...
tabela_final["ValidacaoCep"] = validacao
...
checklist = {
    ...
    "CUSTO DE PGTO encontrado": valor_custo_pgt is not None,
...
# tabela_final.write.format("delta").mode("overwrite").saveAsTable("tabela_cep")
