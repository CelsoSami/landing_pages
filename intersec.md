...
# MAGIC %md
# MAGIC # Transformação EBA_CSC - Aba Audiência
...
# MAGIC %md
# MAGIC ## Leitura da Aba Audiência

# COMMAND ----------

fonte_audiencia = pd.read_excel(
    CAMINHO_ARQUIVO,
    sheet_name="Audiência",
    header=None
)
...
pos_tabela_precos = find_cell(fonte_audiencia, "TABELA DE PREÇOS NO PERIODO")

if pos_tabela_precos is None:
    raise ValueError("Texto 'TABELA DE PREÇOS NO PERIODO' não encontrado na aba Audiência")

linha_inicio_servicos = pos_tabela_precos[0] + 7
coluna_servico = pos_tabela_precos[1] + 1
coluna_peso = coluna_servico + 1

servicos = []
for i in range(linha_inicio_servicos, len(fonte_audiencia)):
    srv_val = fonte_audiencia.iat[i, coluna_servico]
    if pd.isna(srv_val) or str(srv_val).strip() == "":
        break
    servicos.append({
        "Servico": str(srv_val).strip(),
        "Peso": safe_number(fonte_audiencia.iat[i, coluna_peso])
    })
...
pos_rotulos = find_cell(fonte_audiencia, "Rótulos de Linha")

if pos_rotulos is None:
    raise ValueError("Texto 'Rótulos de Linha' não encontrado na aba Audiência")

linha_pivot = pos_rotulos[0]
coluna_pivot = pos_rotulos[1]

linha_cabecalho = fonte_audiencia.iloc[linha_pivot].values
servicos_pivot = linha_cabecalho[coluna_pivot + 1:]

centros_custo = []
for i in range(linha_pivot + 1, len(fonte_audiencia)):
    cc_val = fonte_audiencia.iat[i, coluna_pivot]
    if pd.isna(cc_val):
        continue
    ...
    centros_custo.append({
        "CC": str(cc_val).strip(),
        "row_data": fonte_audiencia.iloc[i].values
    })
...
pos_formacao = find_cell(fonte_audiencia, "FORMAÇÃO DE PREÇOS NO PERIODO")
...
formacao_precos = []
for i in range(linha_formacao_servicos, len(fonte_audiencia)):
    srv_val = fonte_audiencia.iat[i, coluna_formacao_servico]
    if pd.isna(srv_val) or str(srv_val).strip() == "":
        break
    formacao_precos.append({
        "Servico": str(srv_val).strip().upper(),
        "Peso_Formacao": safe_number(fonte_audiencia.iat[i, coluna_formacao_peso])
    })
...
pos_volumetria = find_cell(fonte_audiencia, "VOLUMETRIA NO PERIODO")
...
servicos_vol_raw = fonte_audiencia.iloc[linha_servicos_vol, coluna_inicio_vol:].values
volumetria_raw = fonte_audiencia.iloc[linha_volumetria, coluna_inicio_vol:].values
volumetria_peso_raw = fonte_audiencia.iloc[linha_volumetria_peso, coluna_inicio_vol:].values
...
# MAGIC %md
# MAGIC ## CUSTO DE AUDIÊNCIA (Validação)

# COMMAND ----------

valor_custo_diligencia = get_value_after_label(fonte_audiencia, "CUSTO DE AUDIÊNCIA/DILIGÊNCIA - CORRESPONDENTE:")
valor_custo_csc = get_value_after_label(fonte_audiencia, "CUSTO DE AUDIÊNCIA - CSC (Virtual):")

soma_custo_audiencia = (
    (safe_number(valor_custo_diligencia) or 0)
    +
    (safe_number(valor_custo_csc) or 0)
)

soma_valor_calculado = tabela_final["Valor"].sum()

validacao = (
    round(soma_valor_calculado, 2) == round(soma_custo_audiencia, 2)
    if soma_custo_audiencia is not None
    else False
)

print(f"Custo Diligência/Correspondente: {valor_custo_diligencia}")
print(f"Custo CSC Virtual: {valor_custo_csc}")
print(f"Soma Custo Audiência: {soma_custo_audiencia:.2f}")
...
tabela_final["ValidacaoAudiencia"] = validacao
...
checklist = {
    ...
    "CUSTO DE AUDIÊNCIA/DILIGÊNCIA encontrado": valor_custo_diligencia is not None,
    "CUSTO DE AUDIÊNCIA - CSC encontrado": valor_custo_csc is not None,
...
# tabela_final.write.format("delta").mode("overwrite").saveAsTable("tabela_audiencia")
