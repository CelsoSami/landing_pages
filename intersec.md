...
# MAGIC %md
# MAGIC # Transformação EBA_CSC - Aba Intimação
...
# MAGIC %md
# MAGIC ## Leitura da Aba Intimação

# COMMAND ----------

fonte_intimacao = pd.read_excel(
    CAMINHO_ARQUIVO,
    sheet_name="Intimação",
    header=None
)
...
pos_tabela_precos = find_cell(fonte_intimacao, "TABELA DE PREÇOS NO PERIODO")

if pos_tabela_precos is None:
    raise ValueError("Texto 'TABELA DE PREÇOS NO PERIODO' não encontrado na aba Intimação")

linha_inicio_servicos = pos_tabela_precos[0] + 17
coluna_servico = pos_tabela_precos[1] + 1
coluna_peso = coluna_servico + 1

servicos = []
for i in range(linha_inicio_servicos, len(fonte_intimacao)):
    srv_val = fonte_intimacao.iat[i, coluna_servico]
    if pd.isna(srv_val) or str(srv_val).strip() == "":
        break
    servicos.append({
        "Servico": str(srv_val).strip(),
        "Peso": safe_number(fonte_intimacao.iat[i, coluna_peso])
    })
...
pos_rotulos = find_cell(fonte_intimacao, "Rótulos de Linha")

if pos_rotulos is None:
    raise ValueError("Texto 'Rótulos de Linha' não encontrado na aba Intimação")

linha_pivot = pos_rotulos[0]
coluna_pivot = pos_rotulos[1]

linha_cabecalho = fonte_intimacao.iloc[linha_pivot].values
servicos_pivot = linha_cabecalho[coluna_pivot + 1:]

centros_custo = []
for i in range(linha_pivot + 1, len(fonte_intimacao)):
    cc_val = fonte_intimacao.iat[i, coluna_pivot]
    if pd.isna(cc_val):
        continue
    ...
    centros_custo.append({
        "CC": str(cc_val).strip(),
        "row_data": fonte_intimacao.iloc[i].values
    })
...
pos_formacao = find_cell(fonte_intimacao, "FORMAÇÃO DE PREÇOS NO PERIODO")
...
formacao_precos = []
for i in range(linha_formacao_servicos, len(fonte_intimacao)):
    srv_val = fonte_intimacao.iat[i, coluna_formacao_servico]
    if pd.isna(srv_val) or str(srv_val).strip() == "":
        break
    formacao_precos.append({
        "Servico": str(srv_val).strip().upper(),
        "Peso_Formacao": safe_number(fonte_intimacao.iat[i, coluna_formacao_peso])
    })
...
pos_volumetria = find_cell(fonte_intimacao, "VOLUMETRIA NO PERIODO")
...
servicos_vol_raw = fonte_intimacao.iloc[linha_servicos_vol, coluna_inicio_vol:].values
volumetria_raw = fonte_intimacao.iloc[linha_volumetria, coluna_inicio_vol:].values
volumetria_peso_raw = fonte_intimacao.iloc[linha_volumetria_peso, coluna_inicio_vol:].values
...
# MAGIC %md
# MAGIC ## CUSTO DE INTIMAÇÃO (Validação - 9 rótulos somados)

# COMMAND ----------

rotulos_intimacao = [
    "CUSTO DE INTIMAÇÃO:",
    "CUSTO DE INTIMAÇÃO CAPTURA - GERAL",
    "CUSTO DE INTIMAÇÃO CAPTURA - CNJ - NJ.01",
    "CUSTO DE INTIMAÇÃO CAPTURA - CNJ - NJ.02",
    "CUSTO DE INTIMAÇÃO CAPTURA - CNJ - NJ.03",
    "CUSTO DE INTIMAÇÃO CAPTURA - CNJ - NJ.06",
    "CUSTO DE INTIMAÇÃO CAPTURA - CNJ - NJ.09",
    "CUSTO DE INTIMAÇÃO CAPTURA - CNJ - NJ.12A",
    "CUSTO DE INTIMAÇÃO CAPTURA - CNJ - NJ.18",
]

soma_custo_intimacao = 0
for rotulo in rotulos_intimacao:
    valor = get_value_after_label(fonte_intimacao, rotulo)
    if valor is not None:
        num = safe_number(valor)
        if num is not None:
            soma_custo_intimacao += num
    print(f"  {rotulo}: {valor}")

soma_valor_calculado = tabela_final["Valor"].sum()

validacao = (
    round(soma_valor_calculado, 2) == round(soma_custo_intimacao, 2)
    if soma_custo_intimacao is not None
    else False
)

print(f"\nSoma Custo Intimação: {soma_custo_intimacao:.2f}")
...
tabela_final["ValidacaoIntimação"] = validacao
...
# tabela_final.write.format("delta").mode("overwrite").saveAsTable("tabela_intimacao")
