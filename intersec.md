let
    ...
    Intimação = Fonte{[Item="Intimação",Kind="Sheet"]}[Data],

    // Converte em lista de listas
    Linhas = Table.ToRows(Intimação),
    ...
    LinhaInicioServicos = PosTabelaPrecos[Linha] + 17,
    ...
    // =====================================================
    // CUSTO DE INTIMAÇÃO (aba 'Intimação') - VALIDAÇÃO
    // =====================================================
    ...
    PosCI =
        LocalizarRotulo("CUSTO DE INTIMAÇÃO:"),
    ValorCI =
        ValorMaisProximo(PosCI),

    PosCICG =
        LocalizarRotulo("CUSTO DE INTIMAÇÃO CAPTURA - GERAL"),
    ValorCICG =
        ValorMaisProximo(PosCICG),

    PosC01 =
        LocalizarRotulo("CUSTO DE INTIMAÇÃO CAPTURA - CNJ - NJ.01"),
    ValorC01 =
        ValorMaisProximo(PosC01),

    PosC02 =
        LocalizarRotulo("CUSTO DE INTIMAÇÃO CAPTURA - CNJ - NJ.02"),
    ValorC02 =
        ValorMaisProximo(PosC02),

    PosC03 =
        LocalizarRotulo("CUSTO DE INTIMAÇÃO CAPTURA - CNJ - NJ.03"),
    ValorC03 =
        ValorMaisProximo(PosC03),

    PosC06 =
        LocalizarRotulo("CUSTO DE INTIMAÇÃO CAPTURA - CNJ - NJ.06"),
    ValorC06 =
        ValorMaisProximo(PosC06),

    PosC09 =
        LocalizarRotulo("CUSTO DE INTIMAÇÃO CAPTURA - CNJ - NJ.09"),
    ValorC09 =
        ValorMaisProximo(PosC09),

    PosC12 =
        LocalizarRotulo("CUSTO DE INTIMAÇÃO CAPTURA - CNJ - NJ.12A"),
    ValorC12 =
        ValorMaisProximo(PosC12),

    PosC18 =
        LocalizarRotulo("CUSTO DE INTIMAÇÃO CAPTURA - CNJ - NJ.18"),
    ValorC18 =
        ValorMaisProximo(PosC18),

    // soma dos dois valores encontrados na planilha
    SomaCustoAudiencia =
        (if ValorCI <> null then ValorCI else 0)
        +
        (if ValorCICG <> null then ValorCICG else 0)
        +
        (if ValorC01 <> null then ValorC01 else 0)
        +
        (if ValorC02 <> null then ValorC02 else 0)
        +
        (if ValorC03 <> null then ValorC03 else 0)
        +
        (if ValorC06 <> null then ValorC06 else 0)
        +
        (if ValorC09 <> null then ValorC09 else 0)
        +
        (if ValorC12 <> null then ValorC12 else 0)
        +
        (if ValorC18 <> null then ValorC18 else 0),
    ...
    #"Colunas Renomeadas" = Table.RenameColumns(ComValidacaoAudiencia,{{"ValidacaoAudiencia", "ValidacaoIntimação"}}),
    #"Tipo Alterado2" = Table.TransformColumnTypes(#"Colunas Renomeadas",{{"Volumetria", type number}, {"Volumetria_Com_Peso", type number}, {"Peso_Formacao", type number}})
...
Antes de disponibilizar o arquivo para ingestão, validar:
 ...
 - Existe uma aba denominada exatamente Intimação.
 - Existe uma aba denominada exatamente $.
 - Existe uma célula contendo exatamente o texto "TABELA DE PREÇOS NO PERIODO".
 - Os serviços iniciam exatamente 17 linhas abaixo e 1 coluna à direita da célula "TABELA DE PREÇOS NO PERIODO".
 ...
 - Existe uma célula contendo exatamente o texto "CUSTO DE INTIMAÇÃO:".
 - Existe uma célula contendo exatamente o texto "CUSTO DE INTIMAÇÃO CAPTURA - GERAL".
 - Existe uma célula contendo exatamente o texto "CUSTO DE INTIMAÇÃO CAPTURA - CNJ - NJ.01".
 - Existe uma célula contendo exatamente o texto "CUSTO DE INTIMAÇÃO CAPTURA - CNJ - NJ.02".
 - Existe uma célula contendo exatamente o texto "CUSTO DE INTIMAÇÃO CAPTURA - CNJ - NJ.03".
 - Existe uma célula contendo exatamente o texto "CUSTO DE INTIMAÇÃO CAPTURA - CNJ - NJ.06".
 - Existe uma célula contendo exatamente o texto "CUSTO DE INTIMAÇÃO CAPTURA - CNJ - NJ.09".
 - Existe uma célula contendo exatamente o texto "CUSTO DE INTIMAÇÃO CAPTURA - CNJ - NJ.12A".
 - Existe uma célula contendo exatamente o texto "CUSTO DE INTIMAÇÃO CAPTURA - CNJ - NJ.18".
 - Para cada um dos rótulos acima, o valor do custo deve estar na mesma linha, sendo considerado o primeiro valor preenchido à direita do respectivo rótulo.
...
let
    ...
    Fontex = Fonte,
    Abax = Fontex{[Item="$",Kind="Sheet"]}[Data],
    Linhasx = Table.ToRows(Abax),

    PosBaseFinanceira =
        List.First(
            List.RemoveNulls(
                List.Transform(
                    {0..List.Count(Linhasx)-1},
                    (i) =>
                        let
                            Linha = Linhasx{i},
                            Pos = List.PositionOf(
    ...
    LinhaCompletaBase = Linhasx{LinhaBase},
